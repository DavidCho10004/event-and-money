...
def main():
    ...
    for i, day in enumerate(days, 1):
        ...
        # 외인 지분율 + 종가/시총/상장주식수는 일별로도 수집
        # (1주일 변화폭·종목 상세·강도% 용 — 하루 4회 호출 추가)
        daily_rows = []
        for market in MARKETS:
            frgn = _retry(stock.get_exhaustion_rates_of_foreign_investment, day, market=market)
            cap = _retry(stock.get_market_cap_by_ticker, day, market=market)
            ...
            daily_rows.append(df_s.reset_index())
        if daily_rows:
            store.upsert_snapshots(pd.concat(daily_rows, ignore_index=True))
