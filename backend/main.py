...
SUMMARY_ASSETS_KR = [
    "^KS11",        # KOSPI
    "^KQ11",        # KOSDAQ
    "069500.KS",    # KODEX 200 (KOSPI 200 추종 ETF)
    "091160.KS",    # KODEX 반도체 (한국 핵심 섹터)
    "091180.KS",    # KODEX 자동차
    "305720.KS",    # KODEX 2차전지산업 (핫 테마)
    "102780.KS",    # KODEX 삼성그룹
    "005930.KS",    # 삼성전자
    "035720.KS",    # 카카오
    "152380.KS",    # KODEX 국고채3년 (한국 금리)
    "USDKRW=X",     # 원달러
...
def buy_review_page(request: Request, market: str = Query("KOSPI"),
    ...
    data = get_buy_review(market, p=p, sort=sort, f3=f3, f5=f5, pbr=pbr, inst=inst, indiv=indiv, tri=tri)
...
def api_buy_review(market: str = Query("KOSPI"), p: str = Query("3m"),
                   sort: str = Query("delta"),
                   f3: bool = Query(False), f5: bool = Query(False),
                   pbr: bool = Query(False), inst: bool = Query(False),
                   indiv: bool = Query(False), tri: bool = Query(False)):
    """매수 검토 스크리너 JSON"""
    return JSONResponse(get_buy_review(market, p=p, sort=sort, f3=f3, f5=f5, pbr=pbr, inst=inst, indiv=indiv, tri=tri))
